...
const HOME = "#3e7bd6", DRAW = "#6b7490", AWAY = "#e8b833", ACCENT = "#3e7bd6";

function barBlock(bar: OgBar): string {
  const pct = (n: number) => Math.round(n * 100);
  return `
    <div style="display:flex; flex-direction:column; gap:18px; margin-top:44px;">
      <div style="display:flex; height:30px; border-radius:999px; overflow:hidden; border:1px solid #2a3142;">
...
export async function renderOgPng(opts: OgOptions): Promise<Buffer> {
  const markup = html(`
    <div style="display:flex; flex-direction:column; width:1200px; height:630px; padding:64px 72px; background-color:#0a0c12; color:#ced5e2; font-family:Inter;">
      <div style="display:flex; align-items:center;">
        <div style="display:flex; align-items:center; justify-content:center; width:56px; height:56px; border-radius:14px; background-color:${ACCENT}; color:#03101f; font-size:34px; font-weight:700; font-family:Inter;">26</div>
      ...
      <div style="display:flex; flex:1; flex-direction:column; justify-content:center;">
        ...
        <div style="display:flex; margin-top:14px; font-size:74px; font-weight:700; font-family:Inter; letter-spacing:-2px; line-height:1.05;">${esc(opts.title)}</div>
      ...
      <div style="display:flex; align-items:center; font-size:26px; color:#6f78a0; border-top:1px solid #2a3142; padding-top:24px;">
